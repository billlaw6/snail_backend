...
def trans_locations():
    ...
    for cr in crs:
        ...
        if len(sts) == 0:
            location.country_region_code = cr.get('Code')
            location.country_region_name = cr.get('Name')
            location.country_region_py_code = ''.join(lazy_pinyin(cr.get('Name'), style=pypinyin.FIRST_LETTER))
            location.country_region_quanpin = ''.join(lazy_pinyin(cr.get('Name')))
            data_object.push(location)
